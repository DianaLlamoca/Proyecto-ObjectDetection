# -*- coding: utf-8 -*-
"""CuantizacionModelo_Error.ipynb
...
Cuantización "aware training":

Esta forma de cuantización, según la documentación de TensorFlow, suele ser mejor para la precisión del modelo. Para ello, se aplicará esta forma de cuantización y se calculará el performance.
"""

# Commented out IPython magic to ensure Python compatibility.
#Para ello, se instalará "tensorflow_model_optimization" y, así, aplicar la cuantización "aware training"
# %pip install tensorflow-model-optimization

import tensorflow_model_optimization as tfmot

#Se crea el objeto que cuantizará el modelo
cuantizacion = tfmot.quantization.keras.quantize_model # Fix: Assign the function to the variable

#Se define un quantize_scope para manejar la capa de "rescaling"
with tfmot.quantization.keras.quantize_scope({'Rescaling': tfmot.quantization.keras.quantizers.Quantizer}):
    #Ahora, se cuantiza el modelo con dicho objeto
    modelo_q_aware=cuantizacion(modelo)

#Ahora, se continuará con la compilación (se usarán los mismos parámetros) pero del modelo cuantizado
with strategy.scope():
   modelo_q_aware.compile(loss=func_perd, optimizer=opti,
                   metrics={"cl_head": "accuracy",
                            "bb_head": tensorflow.keras.metrics.MeanSquaredError()})